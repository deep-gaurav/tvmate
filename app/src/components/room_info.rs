                ...
                if let Some(el) = el {
                    ...
                    view! {
                        <Portal mount=element class="h-full w-full bg-black text-white flex flex-col justify-stretch items-center">
                            ...
                            <div> {move||match room_info.with(|r|r.as_ref().map(|r|r.id.to_uppercase())){
                                Some(id) => id,
                                None => "Disconnected".to_string()
                            }} </div>
                            <hr class="border-white border-t w-full"/>
                            <For
                                each=move||room_info.with(|r|r.as_ref().map(|r|r.users.clone())).unwrap_or_default()
                                key=|user|user.id
                                children=|user| {
                                    view! {
                                        <div class="text-left w-full mt-2"> "> " {user.name} </div>
                                    }
                                }
                            />
                        </Portal>
