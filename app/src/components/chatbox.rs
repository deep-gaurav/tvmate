...
pub fn ChatBox() -> impl IntoView {
    let message_history = store_value(Vec::new());

    let (msg_len, set_msg_len) = create_signal(message_history.with_value(|v| v.len()));

    let room_manager = expect_context::<RoomManager>();

    if let Some(message_signal) = room_manager.get_chat_signal() {
        create_effect(move |_| {
            if let Some(msg) = message_signal.get() {
                message_history.update_value(|val| val.push(msg));
